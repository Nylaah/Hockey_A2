"""
Client IA : se connecte au serveur en localhost et joue le rôle RIGHT (ou LEFT).
"""
import socket
import threading
import time
...
class AIClient:
    ...
    def __init__(self, role: str = "RIGHT", username: str = "IA"):
        ...
        # État IA
        self._x  = VWIDTH * 3 / 4 if role == "RIGHT" else VWIDTH / 4
        self._y  = HEIGHT / 2
        self._vx = 0.0
        self._vy = 0.0
    ...
    def start(self):
        ...
        self._running = True
        self._send_raw(f"{self._username}|{self._role}\n")
    ...
    def _move_toward(self, tx: float, ty: float, dt: float):
        ...
        if dist > 8:
            ...
            # Accélération si loin, freinage si proche
